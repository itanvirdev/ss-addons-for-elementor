...
class SS_Testimonial extends Widget_Base {
    ...
    public function get_title() {
        return __('Testimonial', 'ss-addons');
    }
    ...
    protected function register_controls() {
        ...
        $this->start_controls_section(
            'review_list',
            [
                'label' => esc_html__('Review List', 'ss-addons'),
        ...
        $repeater->add_control(
            'reviewer_image',
            [
                'label' => esc_html__('Reviewer Image', 'ss-addons'),
        ...
        $repeater->add_control(
            'reviewer_name',
            [
                'label' => esc_html__('Reviewer Name', 'ss-addons'),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => esc_html__('Rasalina William', 'ss-addons'),
                'label_block' => true,
            ]
        );
        ...
        $repeater->add_control(
            'review_sub',
            [
                'label' => esc_html__('Reviewe Subject', 'ss-addons'),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => esc_html__('Code Quality!', 'ss-addons'),
                'label_block' => true,
            ]
        );
        ...
        $repeater->add_control(
            'reviewer_title',
            [
                'label' => esc_html__('Reviewer Title', 'ss-addons'),
                'type' => \Elementor\Controls_Manager::TEXT,
                'default' => esc_html__('- CEO at YES Germany', 'ss-addons'),
                'label_block' => true,
            ]
        );
        $repeater->add_control(
            'review_content',
            [
                'label' => esc_html__('Review Content', 'ss-addons'),
                ...
                'placeholder' => esc_html__('Type your review content here', 'ss-addons'),
            ]
        );
        ...
        $this->add_control(
            ...
            [
                ...
                'default' => [
                    [
                        'reviewer_name' => esc_html__('Rasalina William', 'ss-addons'),
                        'reviewer_title' => esc_html__('- CEO at YES Germany', 'ss-addons'),
                        'review_content' => esc_html__('Put your trust in us &share in our people with a passion.We are motivated by the satisfaction H.Spond Asset Management is made up of a team of expert, committed and experienced for of clients financial markets. Our goal is to achieve continuous.', 'ss-addons'),
                    ],
                    [
                        'reviewer_name' => esc_html__('Rasalina William', 'ss-addons'),
                        'reviewer_title' => esc_html__('- CEO at YES Germany', 'ss-addons'),
                            ...
                            <div class="row">
                                <?php foreach ($settings['reviews_list'] as $index => $item) :
                                    if (!empty($item['reviewer_image']['url'])) {
                                        $ss_reviewer_image = !empty($item['reviewer_image']['id']) ? wp_get_attachment_image_url($item['reviewer_image']['id'], $settings['thumbnail_size_size']) : $item['reviewer_image']['url'];
                                        $ss_reviewer_image_alt = get_post_meta($item["reviewer_image"]["id"], "_wp_attachment_image_alt", true);
                                    }
                                ?>
                                        ...
                                        <div class="reviewblock reviewblock--style2">
                                            <div class="reviewblock__content">
                                                <div class="reviewblock__author">
                                                    <?php if (!empty($ss_reviewer_image)) : ?>
                                                        <div class="reviewblock__authorImage">
                                                            <img src="<?php echo esc_url($ss_reviewer_image); ?>" alt="<?php echo esc_url($ss_reviewer_image_alt); ?>">
                                                        </div>
                                                    <?php endif; ?>

                                                    <?php if (!empty($item['reviewer_name'])) : ?>
                                                        <h3 class="reviewblock__authorName"><?php echo ss_kses($item['reviewer_name']); ?></h3>
                                                    <?php endif; ?>
                                                    <?php if (!empty($item['review_content'])) : ?>
                                                        <p class="reviewblock__authorSpeech"><?php echo ss_kses($item['review_content']); ?></p>
                                                    <?php endif; ?>
                                                    <?php if (!empty($item['reviewer_title'])) : ?>
                                                        <span class="reviewblock__authorDes"><?php echo ss_kses($item['reviewer_title']); ?></span>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="review__box mb-30">
                                        <div class="reviewblock reviewblock--style3 text-center">
                                            <div class="reviewblock__content">
                                                <div class="reviewblock__author">
                                                    <?php if (!empty($ss_reviewer_image)) : ?>
                                                        <img class="reviewblock__author__image" src="<?php echo esc_url($ss_reviewer_image); ?>" alt="<?php echo esc_url($ss_reviewer_image_alt); ?>">
                                                    <?php endif; ?>

                                                    <?php if (!empty($item['reviewer_name'])) : ?>
                                                        <span class="reviewblock__author__name d-block mb-25"><?php echo ss_kses($item['reviewer_name']); ?></span>
                                                    <?php endif; ?>
                                                    <br>
                                                    <?php if (!empty($item['reviewer_title'])) : ?>
                                                        <span class="reviewblock__authorDes"><?php echo ss_kses($item['reviewer_title']); ?></span>
                                        ...
                                        <div class="testimonial__item transition-3 text-center white-bg">
                                            ...
                                            <div class="testimonial__text">
                                                <?php if (!empty($item['review_sub'])) : ?>
                                                    <h4><?php echo ss_kses($item['review_sub']); ?></h4>
                                                <?php endif; ?>

                                                <?php if (!empty($item['review_content'])) : ?>
                                                    <p><?php echo ss_kses($item['review_content']); ?></p>
                                                <?php endif; ?>

                                            </div>
                                            <div class="testimonial__avater-info mb-5">
                                                <?php if (!empty($item['reviewer_name'])) : ?>
                                                    <h3><?php echo ss_kses($item['reviewer_name']); ?></h3>
                                                <?php endif; ?>
                                                <?php if (!empty($item['reviewer_title'])) : ?>
                                                    <span><?php echo ss_kses($item['reviewer_title']); ?></span>
                                                <?php endif; ?>
                                            </div>
