...
class SS_Advanced_Tab extends Widget_Base {
	...
	public function get_title() {
		return __('Advanced Tab', 'ss-addons');
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			...
			[
				...
				'default' => __('Tab Title', 'ss-addons'),
				'placeholder' => __('Type Tab Title', 'ss-addons'),
				'dynamic' => [
					'active' => true,
				]
			]
		...
		$repeater->add_control(
			'active_tab',
			[
				'label' => __('Is Active Tab?', 'ss-addons'),
		...
		$repeater->add_control(
			'template',
			[
				'label' => __('Section Template', 'ss-addons'),
				'placeholder' => __('Select a section template for as tab content', 'ss-addons'),
...
?>
		...
		?>
			...
			<section class="feature" id="ss-features">
				<div class="container">
					<div class="row">
						<div class="col-12">
								...
								<ul class="nav nav-tabs" id="myTab" role="tablist">
									...
									?>
										<li class="nav-item" role="presentation">
											<button class="nav-link <?php echo esc_attr($active); ?>" id="home-tab-<?php echo esc_attr($key); ?>" data-bs-toggle="tab" data-bs-target="#home-<?php echo esc_attr($key); ?>" type="button" role="tab" aria-controls="home-<?php echo esc_attr($key); ?>" aria-selected="true"><?php echo ss_kses($tab['title']); ?></button>
										</li>
									<?php endforeach; ?>
								</ul>
								...
								<div class="mvvTabs__wrapper d-flex align-items-start">
									<div class="nav nav-pills mb-30" id="v-pills-tab" role="tablist" aria-orientation="vertical">
										<?php foreach ($settings['tabs'] as $key => $tab) :
											$active = ($key == 0) ? 'active' : '';
										?>
											<button class="mvvTabs__button nav-link <?php echo esc_attr($active); ?>" id="v-pills-home-tab-<?php echo esc_attr($key); ?>" data-bs-toggle="pill" data-bs-target="#v-pills-home-<?php echo esc_attr($key); ?>" type="button" role="tab" aria-controls="v-pills-home-<?php echo esc_attr($key); ?>" aria-selected="true"><?php echo ss_kses($tab['title']); ?></button>
										<?php endforeach; ?>
									</div>
