 ...
 * @since 1.0.0
 */
class SS_Live_Donation extends Widget_Base {
    ...
    public function get_title() {
        return __('Live Donation', 'ss-addons');
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'ss_donate_percentage',
            [
                'label' => esc_html__('Percentage', 'ss-addons'),
                'description' => ss_get_allowed_html_desc('basic'),
                'type' => Controls_Manager::TEXT,
                'default' => esc_html__('80', 'ss-addons'),
                'placeholder' => esc_html__('Type Percentage Number', 'ss-addons'),
                'label_block' => true,
            ]
        );
        $this->add_control(
        ...
        $this->add_control(
            'ss_donate_number',
            [
                'label' => esc_html__('Donation Number', 'ss-addons'),
                'description' => ss_get_allowed_html_desc('intermediate'),
                'type' => Controls_Manager::TEXT,
                'default' => esc_html__('28,0000', 'ss-addons'),
                'placeholder' => esc_html__('Type donation number here', 'ss-addons'),
            ]
        );
...
?>
            ...
            <?php if (!empty($settings['ss_section_title_show'])) : ?>
                <div class="contact__info">
                    ...
                    <div class="contact__info__content">
                        <?php if (!empty($settings['ss_donate_percentage'])) : ?>
                            <span class="sub-title ss-el-subtitle"><?php echo ss_kses($settings['ss_donate_percentage']); ?></span>
                        <?php endif; ?>
                        ...
                        <?php if (!empty($settings['ss_donate_number'])) : ?>
                            <span><?php echo ss_kses($settings['ss_donate_number']); ?></span>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
        ...
        ?>
            ...
            <div class="featureTab__content p-0">
                ...
                <?php if (!empty($settings['ss_donate_number'])) : ?>
                    <h3 class="featureTab__content__counter"><?php echo ss_kses($settings['ss_donate_number']); ?></h3>
                <?php endif; ?>
                <?php if (!empty($settings['ss_donate_percentage'])) : ?>
                    <div class="featureBlock__donation__progress">
                        <div class="featureBlock__donation__bar">
                            <span class="featureBlock__donation__text skill-bar skill-bar--text" data-width="<?php echo ss_kses($settings['ss_donate_percentage']); ?>%"><span><?php echo ss_kses($settings['ss_donate_percentage']); ?>%</span></span>
                            <div class="featureBlock__donation__line">
                                <span class="skill-bars">
                                    <span class="skill-bars__line skill-bar" data-width="<?php echo ss_kses($settings['ss_donate_percentage']); ?>%"></span>
                                </span>
                            </div>
                        </div>
                    </div>
...
$widgets_manager->register(new SS_Live_Donation());
